...
namespace mathlib {
	...
	const Vector3f CFrame::RIGHT = Vector3f(1, 0, 0);
	const Vector3f CFrame::UP = Vector3f(0, 1, 0);
	const  Vector3f CFrame::BACK = Vector3f(0, 0, 1);
	...
	Vector3f Vector3f::lerp(Vector3f* other, float t) {
		Vector3f mv = other->mult(t);
		return this->mult((1-t)).add(&mv);
	}
	...
	Vector3f Vector3f::unit() {
		...
		return Vector3f(this->x / m, this->y / m, this->z / m);
	};
	...
	CFrame::CFrame(float nx, float ny, float nz) {
		...
		this->Position = Vector3f(nx, ny, nz);
		this->LookVector = Vector3f(-this->m13, -this->m23, -this->m33);
		this->RightVector = Vector3f(this->m11, this->m21, this->m31);
		this->UpVector = Vector3f(this->m12, this->m22, this->m32);
	};
	...
	CFrame::CFrame(Vector3f* pos) {
		...
		this->Position = Vector3f(pos->x, pos->y, pos->z);
		this->LookVector = Vector3f(-this->m13, -this->m23, -this->m33);
		this->RightVector = Vector3f(this->m11, this->m21, this->m31);
		this->UpVector = Vector3f(this->m12, this->m22, this->m32);
	};
	...
	CFrame::CFrame(Vector3f* pos, Vector3f* lookAt) {
		...
		if (xAxis.magnitude() == 0) {
			if (zAxis.y < 0) {
				xAxis = Vector3f(0, 0, -1);
				yAxis = Vector3f(1, 0, 0);
				zAxis = Vector3f(0, -1, 0);
			} else {
				xAxis = Vector3f(0, 0, 1);
				yAxis = Vector3f(1, 0, 0);
				zAxis = Vector3f(0, 1, 0);
		...
		this->LookVector = Vector3f(-zAxis.x, -zAxis.y, -zAxis.z);
		this->RightVector = Vector3f(xAxis.x, xAxis.y, xAxis.z);
		this->UpVector = Vector3f(yAxis.x, yAxis.y, yAxis.z);
	};
	...
	CFrame::CFrame(float nx, float ny, float nz, float i, float j, float k, float w) {
		...
		this->Position = Vector3f(this->x, this->y, this->z);
		this->LookVector = Vector3f(-this->m13, -this->m23, -this->m33);
		this->RightVector = Vector3f(this->m11, this->m21, this->m31);
		this->UpVector = Vector3f(this->m12, this->m22, this->m32);
	};
	...
	CFrame::CFrame(float n14, float n24, float n34, float n11, float n12, float n13, float n21, float n22, float n23, float n31, float n32, float n33) {
		...
		this->Position = Vector3f(this->m14, this->m24, this->m34);
		this->LookVector = Vector3f(-this->m13, -this->m23, -this->m33);
		this->RightVector = Vector3f(this->m11, this->m21, this->m31);
		this->UpVector = Vector3f(this->m12, this->m22, this->m32);
	};
	...
	void CFrame::toDefault() {
		...
		this->LookVector = Vector3f(0, 0, -1);
		this->UpVector = Vector3f(1, 0, 0);
		this->RightVector = Vector3f(0, 1, 0);
	};
	...
	Vector3f CFrame::mult(Vector3f* other) {
		...
		Vector3f right = Vector3f(m11, m21, m31).mult(other->x);
		Vector3f up = Vector3f(m12, m22, m32).mult(other->y);
		Vector3f back = Vector3f(m13, m23, m33).mult(other->z);
	...
	CFrame CFrame::mult(CFrame* other) {
		...
		return CFrame(n14, n24, n34, n11, n12, n13, n21, n22, n23, n31, n32, n33);
	};
	...
	CFrame CFrame::sub(Vector3f* other) {
		CFrameComponents ac = this->components();
		return CFrame(ac.x - other->x, ac.y - other->y, ac.z - other->z, ac.m11, ac.m12, ac.m13, ac.m21, ac.m22, ac.m23, ac.m31, ac.m32, ac.m33);
	};
	...
	CFrame CFrame::invert4x4(CFrame* a) {
		...
		return CFrame(b14, b24, b34, b11, b12, b13, b21, b22, b23, b31, b32, b33);
	};
	...
	CFrame CFrame::lerpinternal(CFrame* a, CFrame* b, float t) {
		...
		Vector3f v = Vector3f(q.i, q.j, q.k);
	...
	CFrame CFrame::fromAxisAngle(Vector3f* axis, float theta) {
		...
		return CFrame(0, 0, 0, r.x, u.x, b.x, r.y, u.y, b.y, r.z, u.z, b.z);
	};
